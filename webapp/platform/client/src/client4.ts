...
export default class Client4 {
    ...
    upsertGuestDraft = async (draft: Draft, connectionId: string) => {
        const result = await this.doFetch<Draft>(
            `${this.getDraftsRoute()}/guest`,
            {
                method: 'post',
                body: JSON.stringify(draft),
                headers: {
    ...
    getDrafts = (teamId: Team['id']) => {
        // Alias for getUserDrafts for consistency
        return this.getUserDrafts(teamId);
    };
    ...
    CreateDraft = async (draft: Draft) => {
        // Helper method for creating drafts with automatic connection ID
        const connectionId = Date.now().toString();
        return this.upsertDraft(draft, connectionId);
    };
